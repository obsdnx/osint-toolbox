...
| Tab | What it does |
|---|---|
| **★ FOOTPRINT** | The headline aggregator. Enter any identifiers → it runs every source, **fetches and scrapes the discovered pages/endpoints**, extracts the actual data out of them (emails, phones, real names, locations, linked accounts via regex + JSON-LD structured data), correlates it with per-fact source counts, and **pivots** on newly-discovered handles. Streams live over SSE and produces one consolidated dossier of what's genuinely public about you, exportable to a printable report. |
